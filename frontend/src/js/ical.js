...
var PolyRegression = {
    ...
    newrow: function (values) {
        ...
        td4.innerHTML = this.plato
            ? values[1].toFixed(2)
            : values[1].toFixed(4); // BrewMath.brix2sg(this.points[i][1]).toFixed(3);

        var td6i = tr.querySelector("td.pl_value");
        td6i.innerHTML = this.plato
            ? values[2].toFixed(2)
            : values[2].toFixed(4);

        var td6 = tr.querySelector("td.pl_error");
        td6.innerHTML = this.plato
            ? values[3].toFixed(2)
            : (values[3] * 1000).toFixed(1);

        var ig_input = tr.querySelector("input.pl_ignored_cb");
        ig_input.checked = values[4];
        var t = this;
        ig_input.onchange = function () {
            t.igchanged(this);
        };
        return tr;
    },
    ...
    getFormula: function () {
        ...
        const poly = regression.polynomial(
            t.points,
            t.points.length > 3 ? 3 : t.points.length > 2 ? 2 : 1,
            {
                precision: 9,
            },
        );
        ...
        t.sgByTilt =
            t.points.length > 3
                ...
                : t.points.length > 2
                  ...
                  : function (x) {
                        return poly.equation[0] + poly.equation[1] * x;
                    };
    ...
    chart: function () {
        ...
        if (typeof this.graph == "undefined") {
            this.graph = new Dygraph(byId("graph"), data, {
