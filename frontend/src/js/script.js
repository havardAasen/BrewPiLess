    ...
    for (var i = 0; i < statePatterns.length; i++) {
        ...
        if (match) {
            if (typeof match[1] !== "undefined")
                status.ControlStateSince = match[1];
            break;
        }
    }
...
function renderLcdText(info) {
    ...
    function genStateText(state, duration) {
        ...
        if ((match = /(\d+)h(\d\d)m(\d\d)/.exec(duration))) {
            timestr = "<%= time_format_long %>"
                .replace("{SS}", match[3])
                .replace("{MM}", match[2])
                .replace("{HH}", match[1]);
        } else if ((match = /(\d+)m(\d\d)/.exec(duration))) {
            // short
            timestr = "<%= time_format_short %>"
                .replace("{SS}", match[2])
                .replace("{MM}", match[1]);
        }
...
function gravityDevice(msg) {
    ...
    var lu;
    if (typeof msg["lu"] != "undefined") lu = new Date(msg.lu * 1000);
    else lu = new Date();
    ...
    //}
    if (typeof msg["lpf"] != "undefined") gravityFilter.setBeta(msg["lpf"]);

    if (typeof msg["stpt"] != "undefined")
        gravityTracker.setThreshold(msg["stpt"]);

    if (typeof msg["ctemp"] != "undefined") window.caltemp = msg["ctemp"];
}
...
function inputsg_change() {
    ...
    if (window.plato) {
        const correctedSg = BrewMath.pTempCorrection(
            window.celsius,
            gravity,
            temp,
            caltemp,
        ...
        const correctedSg = BrewMath.tempCorrection(
            window.celsius,
            gravity,
            temp,
            caltemp,
    ...
    if (typeof window.beerTemp != "undefined") {
        ...
        if (window.plato) {
            const sgc = BrewMath.pTempCorrection(
                window.celsius,
                gravity,
                temp,
                window.beerTemp,
            ...
            const sgc = BrewMath.tempCorrection(
                window.celsius,
                gravity,
                temp,
                window.beerTemp,
...
function wifibar(did, x) {
    ...
    for (let i = 0; i < bars.length; i++) {
        bars[i].style.backgroundColor =
            i < bar ? window.rssiBarColor : "rgba(255,255,255,0.05)";
    }
...
function displayrssi(x) {
    select("#rssi").title =
        x > 0 ? "?" : Math.min(Math.max(2 * (x + 100), 0), 100);
    wifibar("#rssi", x);
    if (select("#wifisignal"))
        select("#wifisignal").innerHTML =
            x > 0 ? "?" : Math.min(Math.max(2 * (x + 100), 0), 100);
}

function initRssi() {
    var rssi = select("#rssi");
    window.rssiBarColor = window
        .getComputedStyle(select(".rssi-bar1"))
...
function ptcshow(msg) {
    if (typeof msg["ptc"] == "undefined" || typeof msg["pt"] == "undefined")
        return;
    var mode = msg.ptc;
    var time = msg.pt;
    ...
    if (state)
        state.style.backgroundColor = mode == "c" ? "lightgreen" : "gray";
    ...
    if (typeof msg["ptctp"] != "undefined") {
        ...
        if (temp)
            temp.innerHTML =
                msg.ptctp < -100 ? "NA" : msg.ptctp / 100 + "&deg;C";
    }
    if (
        typeof msg["ptclo"] != "undefined" &&
        typeof msg["ptcup"] != "undefined"
    ) {
